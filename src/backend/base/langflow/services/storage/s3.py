...
class S3StorageService(StorageService):
    ...
    async def list_files(self, flow_id: str):
        """List all files in a specified flow_id of the S3 bucket.

        Args:
            flow_id: The flow_id in the bucket to list files from.
        ...
        """
        try:
            response = self.s3_client.list_objects_v2(Bucket=self.bucket, Prefix=flow_id)
        except ClientError:
            logger.exception(f"Error listing files in flow_id {flow_id}")
            raise

        files = [item["Key"] for item in response.get("Contents", []) if "/" not in item["Key"][len(flow_id) :]]
        logger.info(f"{len(files)} files listed in flow_id {flow_id}.")
        return files
